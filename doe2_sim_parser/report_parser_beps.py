...
(?P<name>.+)
\s{2}
(?P<type_>ELECTRICITY|NATURAL\-GAS)
''',
    flags=re.VERBOSE
)
...
def parse_no_by_category(line: str) -> OrderedDict:
    ...
    return OrderedDict(
        (*_.groupdict().items(),
         *starmap(lambda x, y: (x, float(y)),
                  zip(Categories, _.groups()[1:]))))


def parse_content(lines: List[str]) -> Tuple:
    return tuple(map(lambda x: (parse_meter(x[0]),
                                parse_no_by_category(x[1])),
                     chunks(list(filter(lambda x: x.strip(), lines)), 2)))


def parse_sum(lines: List[str]) -> Tuple:
    return tuple(map(lambda x: parse_no_by_category(x),
                     lines))


def parse_total(lines: List[str]):
    return tuple(map(lambda x: pattern_total_energy.search(x).groups(),
                     lines))
...
def write_sum(ws: Worksheet, cell: Cell, sum_: List[str]):
    ...
    for _ in sum_:
        for i, pair in enumerate(_.items()):
            k, v = pair
            ws.cell(row=_cell.row,
                    column=_cell.col_idx + 2 + i).value = v
        _cell = ws.cell(row=_cell.row + 1, column=1)
    return _cell
...
SLICES_BEPS = (
    SliceFunc(name='header', slice=slice(0, 3), func_parse=lambda x: x,
              func_write=None),
    SliceFunc(name='categories', slice=slice(5, 7),
    ...
    SliceFunc(name='note', slice=slice(-3, -2), func_parse=lambda x: x,
              func_write=write_note)
)


def parse_beps(report: List[str]):
    yield from map(lambda x: Write(result=x.func_parse(report[x.slice]),
                                   func_write=x.func_write),
                   SLICES_BEPS)
